...
- **Move a workout** — drag a day's tile onto another day of the same training week (or use *Move to…* on the tile's menu) to trade the two around. The week keeps its shape and its total load; race day cannot move.
- **Strava sync** — connect Strava and auto-log activities into the matching training day. A session done on the wrong day still finds the slot it was meant for, weights and HIIT classes complete cross-training days, and only running ever counts as mileage.
